...
public class SelectAndModifyData extends JFrame {
	...
	private void button2ActionPerformed(ActionEvent e) {
		 ...
		 */
		db.sqlLines = "update 船只检验证书 set 船名=?,船检登记号=?,检验证编号=?,船舶所有人=?,船舶登记号=?,船舶检验类型=?,下次检验时间=?,通知时间=?,检验机关=?,检验证使用有效期至=?,发证日期=? where 序号=?";
		db.pre();
		try {
			for (int i = 1; i <= 11; i++) {
				db.preparedStatement.setString(i, table1.getValueAt(table1.getSelectedRow(), i).toString());
			}
			db.preparedStatement.setString(12, table1.getValueAt(table1.getSelectedRow(), 0).toString());
		} catch (SQLException e1) {
			e1.printStackTrace();
		}
		db.exeSql();
