...
public class SelectAndModifyData extends JFrame {
	...
	public void addCheckBoxItems() {
		...
		comboBox1.addItem("下次检查时间");
		comboBox1.addItem("通知时间");
	}
	...
	private void button1ActionPerformed(ActionEvent e) {
		...
		if (comboBox1.getSelectedIndex() == 3 || comboBox1.getSelectedIndex() == 4) {
			db.sqlLines = "select *from 船只安检证书 where 下次检查时间=? or 通知时间=?";
			db.pre();
			try {
	...
	private void button2ActionPerformed(ActionEvent e) {
		 ...
		 */
		db.sqlLines = "update 船只安检证书 set 船名=?,船检登记号=?,安检证书编号=?,船舶所有人=?,船舶登记号=?,检查机关=?,下次检查时间=?,通知时间=?,安检证书使用有效期至=?,发证日期=? where 序号=?";
		db.pre();
		try {
			for (int i = 1; i <= 10; i++) {
		...
		try {
			...
			db.preparedStatement.setString(5, "船只安检证书");
		} catch (SQLException e1) {
			e1.printStackTrace();
		}
		db.exeSql();
	...
	private void button3ActionPerformed(ActionEvent e) {
		/**
		 * @description: 查询[船只安检证书历史表]
		 * @param [e]
		 * @return void
		 * @date: 2018/11/17 10:24
		 */
		db.sqlLines = "select * from 船只安检证书历史表 where 船名=? ";
		db.pre();
		try {
	...
	private void initComponents() {
		...
		// ======== panel1 ========
		{
			...
			panel1.add(label4, new GridBagConstraints(1, 6, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.BOTH, new Insets(0, 0, 5, 5), 0, 0));
			panel1.add(textField2, new GridBagConstraints(2, 6, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.BOTH, new Insets(0, 0, 5, 5), 0, 0));
			// ---- button3 ----
